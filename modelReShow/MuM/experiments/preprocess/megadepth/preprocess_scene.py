...
for image_name in image_names:
    ...
    #     print('Image path exists:', image_path)
    # else:
    ...
    if os.path.exists(image_path):
        ...
        # print('Depth path not found:', depth_path)
        # depth_paths.append(None)
        image_paths.append(None)
        print('WARNING: Image path not found:', image_path)
...
for idx1 in range(n_images):
    if image_paths[idx1] is None:
        continue
    for idx2 in range(idx1 + 1, n_images):
        if image_paths[idx2] is None:
            continue
        matches = (
...
# print('Image paths:', image_paths)  # Debugging line to check the first few image paths
