...
root = Path("/mimer/NOBACKUP/groups/3d-dl/3D_Street_View_data/dataset_aligned/")
...
for sequence_name in os.listdir(root):
    sequence_path = root / sequence_name
    ...
    if sequence_path.exists():
        ...
        files = [
            ...
            if os.path.isfile(sequence_path / f) and f.lower().endswith((".jpg", ".jpeg"))
        ]
...
with gzip.open(f"{output_root}/annotations/3DStreetView.jgz", "wt", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=4)

total_images = sum(len(v) for v in out.values())
print(f"\nProcessed {len(out)} sequences with a total of {total_images} images.")
print(f"Output saved to {output_root}/annotations/3DStreetView.json and 3DStreetView.jgz")