"""DeFusion++ single-modality self-supervised (CUD) training model.

Clean reconstruction of the historical ``UCMIModel.py`` (commit 136f957).

Stage 1 (CUD, "Common-Unique Decomposition") trains on a single source image
that is split into two randomly-masked views.  The model learns to decompose
the image into a common part and two unique parts and to fuse them back.

``forward(img1, img2)`` returns images in [0, 1]:

    (rec_img, rec_img, com_img, uni_img1, uni_img2, fuse_img)
"""
import torch
...
class UCMIMNet(nn.Module):
    ...
    def unpatchifyc(self, x, c=3, p=16):
        """x: (N, L, p*p*c) -> imgs: (N, c, H, W), square grid."""
        h = w = int(x.shape[1] ** .5)
    ...
    def forward(self, img1, img2):
        ...
        # Masked Feature Modeling (MFM) branch
